# Changelog

## reactRouter 0.2.0

- CRAN submission
- update React Router to v.7
- added components from React Router v.7 as R functions, including
  hooks.
- BREAKING CHANGE: `RouterProvider` use “router” argument with
  `create*Router()` functions.
- warning message regarding BREAKING CHANGE `reloadDocument`: now FALSE
  by default

## reactRouter 0.1.2

- BREAKING CHANGE: `reloadDocument` is now FALSE by default (like in
  React Router 6.30.0)
- added data loader and hooks
- improve examples and docs

## reactRouter 0.1.1

CRAN release: 2025-07-23

- fix [\#1](https://github.com/lgnbhl/reactRouter/issues/1)
- add more examples

## reactRouter 0.1.0

CRAN release: 2025-04-29

- initial commit
