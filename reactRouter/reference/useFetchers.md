...
Calls the `useFetchers()` hook and injects the result (or a `selector`
mapped over each fetcher) `as` a prop of the `into` component. Returns
an array of all active fetchers. Useful for showing a global loading
indicator.
