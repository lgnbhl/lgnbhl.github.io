# Package index

## All functions

- [`Await()`](https://felixluginbuhl.com/reactRouter/reference/Await.md)
  : Await
- [`BrowserRouter()`](https://felixluginbuhl.com/reactRouter/reference/BrowserRouter.md)
  : BrowserRouter
- [`createBrowserRouter()`](https://felixluginbuhl.com/reactRouter/reference/createBrowserRouter.md)
  : createBrowserRouter
- [`createHashRouter()`](https://felixluginbuhl.com/reactRouter/reference/createHashRouter.md)
  : createHashRouter
- [`createMemoryRouter()`](https://felixluginbuhl.com/reactRouter/reference/createMemoryRouter.md)
  : createMemoryRouter
- [`createRoutesFromElements()`](https://felixluginbuhl.com/reactRouter/reference/createRoutesFromElements.md)
  : createRoutesFromElements
- [`Form()`](https://felixluginbuhl.com/reactRouter/reference/Form.md) :
  Form
- [`HashRouter()`](https://felixluginbuhl.com/reactRouter/reference/HashRouter.md)
  : HashRouter
- [`Link()`](https://felixluginbuhl.com/reactRouter/reference/Link.md)
  [`Link.shinyInput()`](https://felixluginbuhl.com/reactRouter/reference/Link.md)
  [`updateLink.shinyInput()`](https://felixluginbuhl.com/reactRouter/reference/Link.md)
  : Link
- [`MemoryRouter()`](https://felixluginbuhl.com/reactRouter/reference/MemoryRouter.md)
  : MemoryRouter
- [`Navigate()`](https://felixluginbuhl.com/reactRouter/reference/Navigate.md)
  : Navigate
- [`NavLink()`](https://felixluginbuhl.com/reactRouter/reference/NavLink.md)
  [`NavLink.shinyInput()`](https://felixluginbuhl.com/reactRouter/reference/NavLink.md)
  [`updateNavLink.shinyInput()`](https://felixluginbuhl.com/reactRouter/reference/NavLink.md)
  : NavLink
- [`Outlet()`](https://felixluginbuhl.com/reactRouter/reference/Outlet.md)
  : Outlet
- [`print(`*`<reactRouter>`*`)`](https://felixluginbuhl.com/reactRouter/reference/print.reactRouter.md)
  : Print reactRouter components
- [`reactRouterDependency()`](https://felixluginbuhl.com/reactRouter/reference/reactRouterDependency.md)
  : react-router-dom JS dependency
- [`reactRouterExample()`](https://felixluginbuhl.com/reactRouter/reference/reactRouterExample.md)
  : Run reactRouterExample example
- [`Route()`](https://felixluginbuhl.com/reactRouter/reference/Route.md)
  : Route
- [`RouterProvider()`](https://felixluginbuhl.com/reactRouter/reference/RouterProvider.md)
  : RouterProvider
- [`Routes()`](https://felixluginbuhl.com/reactRouter/reference/Routes.md)
  : Routes
- [`ScrollRestoration()`](https://felixluginbuhl.com/reactRouter/reference/ScrollRestoration.md)
  : ScrollRestoration
- [`useActionData()`](https://felixluginbuhl.com/reactRouter/reference/useActionData.md)
  : useActionData
- [`useBlocker()`](https://felixluginbuhl.com/reactRouter/reference/useBlocker.md)
  : useBlocker
- [`useFetcher()`](https://felixluginbuhl.com/reactRouter/reference/useFetcher.md)
  : useFetcher
- [`useFetchers()`](https://felixluginbuhl.com/reactRouter/reference/useFetchers.md)
  : useFetchers
- [`useHref()`](https://felixluginbuhl.com/reactRouter/reference/useHref.md)
  : useHref
- [`useLoaderData()`](https://felixluginbuhl.com/reactRouter/reference/useLoaderData.md)
  : useLoaderData
- [`useLocation()`](https://felixluginbuhl.com/reactRouter/reference/useLocation.md)
  : useLocation
- [`useMatch()`](https://felixluginbuhl.com/reactRouter/reference/useMatch.md)
  : useMatch
- [`useMatches()`](https://felixluginbuhl.com/reactRouter/reference/useMatches.md)
  : useMatches
- [`useNavigation()`](https://felixluginbuhl.com/reactRouter/reference/useNavigation.md)
  : useNavigation
- [`useNavigationType()`](https://felixluginbuhl.com/reactRouter/reference/useNavigationType.md)
  : useNavigationType
- [`useOutletContext()`](https://felixluginbuhl.com/reactRouter/reference/useOutletContext.md)
  : useOutletContext
- [`useParams()`](https://felixluginbuhl.com/reactRouter/reference/useParams.md)
  : useParams
- [`useResolvedPath()`](https://felixluginbuhl.com/reactRouter/reference/useResolvedPath.md)
  : useResolvedPath
- [`useRevalidator()`](https://felixluginbuhl.com/reactRouter/reference/useRevalidator.md)
  : useRevalidator
- [`useRouteError()`](https://felixluginbuhl.com/reactRouter/reference/useRouteError.md)
  : useRouteError
- [`useRouteLoaderData()`](https://felixluginbuhl.com/reactRouter/reference/useRouteLoaderData.md)
  : useRouteLoaderData
- [`useSearchParams()`](https://felixluginbuhl.com/reactRouter/reference/useSearchParams.md)
  : useSearchParams
