...
- fetcherKey:
  ...
  Character. Optional key to share a fetcher across components (e.g.
  `"my-fetcher"`).
...
Calls the `useFetcher()` hook and injects the result (or a `selector`
from it) `as` a prop of the `into` component. Use to fetch data or
