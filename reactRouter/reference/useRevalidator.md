...
`selector` from it) `as` a prop of the `into` component. Returns the
revalidation `state` (`"idle"` or `"loading"`). Useful for showing
loading feedback during manual or polling revalidation.
