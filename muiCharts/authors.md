# Authors and Citation

## Authors

- **Félix Luginbuhl**. Author, maintainer.

## Citation

Source:
[`DESCRIPTION`](https://github.com/lgnbhl/muiCharts/blob/HEAD/DESCRIPTION)

Luginbuhl F (2026). *muiCharts: 'MUI X Charts' for 'shiny' Apps and
'Quarto'*. R package version 0.1.0,
<https://github.com/lgnbhl/muiCharts>.

    @Manual{,
      title = {muiCharts: 'MUI X Charts' for 'shiny' Apps and 'Quarto'},
      author = {Félix Luginbuhl},
      year = {2026},
      note = {R package version 0.1.0},
      url = {https://github.com/lgnbhl/muiCharts},
    }
