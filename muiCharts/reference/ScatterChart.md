...
- series `Array<object>`\
  Default is - The series to display in the scatter chart. An array of
  ScatterSeries objects.

- axisHighlight
  `x?: 'band', 'line', 'none', y?: 'band', 'line', 'none'`\
  Default is x: 'none', y: 'none' The configuration of axes
  highlight.See highlighting docs for more details.
...
- disableVoronoi `bool`\
  Default is FALSE If true, the interaction will not use the Voronoi
  cell and fall back to hover events.
