...
The Data Grid can automatically adjust its height to fit the content by
using the `autoHeight` prop:
...
You can add pagination and adjust the page size using `initialState`
(here 5 rows per page):
...
filter, density, and export controls. The **Export** button can download
the current data as a CSV file.
