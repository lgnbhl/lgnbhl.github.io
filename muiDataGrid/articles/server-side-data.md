...
> **Experimental — specific to this package.**
...
> have **no equivalent in MUI X Data Grid**. MUI ships only the building
> blocks (`paginationMode = "server"` plus callbacks) and leaves the
...
By default,
[`DataGrid()`](https://felixluginbuhl.com/muiDataGrid/reference/DataGrid.md)
sends **all rows** to the browser at once. Pagination, sorting, and
filtering are handled entirely in JavaScript on the client side. This
works well for small datasets, but becomes a problem with large ones:

- **Performance**: sending thousands of rows as JSON slows down the
  initial page load.
- **Memory**: the browser must hold the entire dataset in memory.
- **Security**: all data is accessible in the browser, even rows not
  currently displayed.

With **server-side data**, only the rows for the current page are sent
to the browser. Pagination, sorting, and filtering are handled in R on
the server. This keeps the app fast regardless of dataset size.

### Client-side vs server-side

|  | [`DataGrid()`](https://felixluginbuhl.com/muiDataGrid/reference/DataGrid.md) (client-side) | [`DataGridServer()`](https://felixluginbuhl.com/muiDataGrid/reference/DataGridServer.md) (server-side) |
|----|----|----|
...
[`DataGridServer()`](https://felixluginbuhl.com/muiDataGrid/reference/DataGridServer.md)
is a Shiny-only component. It uses a custom React component that:

1.  Manages pagination, sort, and filter state internally in the
    browser.
2.  Sends the current state to R via `input$<inputId>` whenever the user
    changes page, sorts a column, or applies a filter.
...
The toolbar **quick filter** (`showToolbar = TRUE`) is also supported:
each search term must appear — as a case-insensitive substring — in at
least one column other than `id`; terms combine per the model’s
`quickFilterLogicOperator` (`"and"` by default), and the result is
combined with the regular filter items using AND.

Unrecognized operators pass all rows through (no filtering) **and emit a
warning**. If you need operators not listed here, use manual mode and
...
> case-sensitive, matching MUI’s default behavior. Filter items with an
> empty value (cleared text input, all `isAnyOf` chips removed, boolean
> “any”) are ignored, matching MUI’s client-side behavior. Because
> filtering is reimplemented in R, edge cases (locale collation,
> accents) can differ from MUI’s own client-side filter.
...
- **Mode is chosen by `rowCount`.** Supplying `rowCount` switches to
  manual mode and `rows` is treated as an already-sliced page — passing
  the *full* dataset with `rowCount` set will show everything on page
  one with no pagination. Omit `rowCount` for automatic mode. (As a
  guard,
  [`DataGridServer()`](https://felixluginbuhl.com/muiDataGrid/reference/DataGridServer.md)
  warns if `nrow(rows)` exceeds `rowCount`.)
- **Sorting or filtering resets to the first page.**
- **Missing values always sort last**, regardless of sort direction.
- **Row ids must be stable for selection.** When `rows` has no `id`
  column, ids are generated positionally (1..n of the *current*
  filtered/sorted result), so the same record gets a different id after
  a sort or filter change. If you use `checkboxSelection` with
  server-side data, include a stable, unique `id` column in `rows` so
  selections survive sorting and filtering.
...
The grid state is available in `input$<inputId>` as a list with three
elements:
...
- `filter_model`: list with `items`, each containing `field`,
  `operator`, and `value`; `logicOperator` (“and” or “or”); and, when
  the toolbar quick filter is used, `quickFilterValues`
...
[`processGridParams()`](https://felixluginbuhl.com/muiDataGrid/reference/processGridParams.md)
covers common sorting and filtering operators. If you need custom logic
(e.g. querying a database), you can skip it and handle `input$<inputId>`
directly:
