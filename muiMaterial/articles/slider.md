...
page](https://mui.com/material-ui/react-slider/).
...
shiftStep prop (the granularity with which the slider can step when
using Page Up/Down or Shift + Arrow Up/Down) to a value divisible by the
step.
...
an array of values to the value prop.
...
Set the orientation prop to “vertical” to create vertical sliders. The
...
Chrome versions below 124 implement aria-orientation incorrectly for
vertical sliders and expose them as ‘horizontal’ in the accessibility
tree. (Chromium issue \#40736841)

The -webkit-appearance: slider-vertical CSS property can be used to
correct this for these older versions, with the trade-off of causing a
console warning in newer Chrome versions:
...
You can customize your slider by adding and repositioning marks for
...
In the following demo, the value x represents the value 2^x. Increasing
x by one increases the represented value by factor 2.
...
- Each thumb has a user-friendly text for its current value. This is not
  ...
  change the name with the getAriaValueText or aria-valuetext prop.

## Unstyled

Use the Base UI Slider for complete ownership of the component’s design,
with no Material UI or Joy UI styles to override. This unstyled version
of the component is the ideal choice for heavy customization with a
smaller bundle size.
