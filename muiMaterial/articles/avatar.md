...
Avatars are fundamental UI elements used to represent users throughout
Material Design interfaces. They appear in tables, menus, dialogs, and
more. The Avatar component can display images, text initials, or icons.

## Image avatars

Image avatars are created by passing `src` and `alt` properties to the
...
Control avatar size using the `sx` prop with `width` and `height` CSS
properties:
...
to stack multiple avatars. Control the maximum number displayed with the
`max` prop:
...
See the [Material UI Avatar
documentation](https://mui.com/material-ui/api/avatar/) and [AvatarGroup
documentation](https://mui.com/material-ui/api/avatar-group/) for
