...
The Select component provides a dropdown menu for selecting one or more
...
**Use
[`Select()`](https://felixluginbuhl.com/muiMaterial/reference/Select.md)
when:** - You have a short, fixed list of options - Users need to see
all options at a glance - No search/filtering is needed - You want a
simpler, more compact interface

**Use
[`Autocomplete()`](https://felixluginbuhl.com/muiMaterial/reference/Autocomplete.md)
when:** - You have a data.frame with a long list of options -
Search/filtering functionality is beneficial - Users might not know the
exact option they want - You need grouped options with search - Multiple
selection with tag display is important

## Basic Usage

Use
[`Select.shinyInput()`](https://felixluginbuhl.com/muiMaterial/reference/Select.md)
to create a dropdown select with options provided as
...
Select supports three visual variants: `outlined` (default), `filled`,
and `standard`.
...
Use the `inputProps` argument with a `label` property to add a label to
the Select.
...
Set `multiple = TRUE` to allow selecting multiple values. The selected
...
Use `displayEmpty = TRUE` to show a placeholder even when no value is
selected.
...
Set `autoWidth = TRUE` to automatically size the dropdown menu based on
its content.
...
Use the `sx` argument for custom styling and control the appearance of
...
Set `native = TRUE` to use the browser’s native select element (useful
for mobile devices).
...
**Note:** When using `native = TRUE`, use HTML `option()` elements
instead of
[`MenuItem()`](https://felixluginbuhl.com/muiMaterial/reference/MenuItem.md)
components.
