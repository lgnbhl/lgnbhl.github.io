...
The Badge component is used to display a small notification or count
next to another element. It’s commonly used for notification counts,
status indicators, or other small pieces of information.
...
The `variant = "dot"` prop changes a badge into a small dot. This can be
...
You can use the `overlap` prop to place the badge relative to the corner
of the wrapped element. Use `"rectangular"` (default) for rectangular
...
You can use the `anchorOrigin` prop to move the badge to any corner of
the wrapped element.
...
You should provide a full description for badges when displaying counts,
especially for important notifications. Use `aria-label` on the parent
