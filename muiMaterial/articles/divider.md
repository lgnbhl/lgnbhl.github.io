...
elements to reinforce visual hierarchy. It renders as a dark gray `<hr>`
by default and features several useful props for quick style
adjustments.
...
Use the `orientation` prop to change the Divider from horizontal to
vertical. When using vertical orientation, the Divider renders a `<div>`
...
Use the `flexItem` prop to display the Divider when it’s being used in a
flex container:
...
Use the Divider with child elements to create dividers with text or
other content:
...
Use the `textAlign` prop to align elements that are wrapped by the
Divider:
...
prop to render it as an `<li>`—otherwise it won’t be a valid HTML
element:
...
Due to its implicit role of `separator`, the Divider (which is an `<hr>`
element) will be announced by screen readers as a “Horizontal Splitter”
(or vertical if using the `orientation` prop).
...
element to a `<div>` using the `component` prop and set
`role="presentation"`:
...
See the [Material UI Divider
documentation](https://mui.com/material-ui/api/divider/) for complete
API details.
