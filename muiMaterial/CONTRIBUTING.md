...
1.  Add a `<Name> <- component('<Name>')` entry to `R/components.R` (use
    `module = '@mui/lab'` for lab components), and a matching roxygen
    doc block in `R/documentation.R`.
...
3.  Add a `tests/testthat/test-<Name>.R` test, and confirm the name is
    actually exported by the bundled MUI version (a wrapper pointing at
    a non-existent export renders to an “Element type is invalid”
    error).
