...
- getItemChildren `func`\
  Default is (item) = item.children Used to determine the children of a
  given item.Signature:function(item: R) = Arrayitem The item to check.
...
- getItemLabel `func`\
  Default is (item) = item.label Used to determine the string label for
  a given item.Signature:function(item: R) = stringitem The item to
  check.
...
- isItemDisabled `func`\
  ...
  disabled.Signature:function(item: R) = booleanitem The item to check.
...
- onItemLabelChange `func`\
  ...
  voiditemId The id of the item that was edited.newLabel The new label
  of the items.
...
- onSelectedItemsChange `func`\
  ...
  itemIds: Array \| string) = voidevent The DOM event that triggered the
  change. Can be null when the change is caused by the
...
- selectionPropagation `{ descendants?: bool, parents?: bool }`\
  ...
  automatically.Only works when multiSelect is true. On the
  SimpleTreeView / , only the expanded items are considered (since the
  collapsed item are not passed to the Tree View component at all)
